...
后台用于管理小程序的主题、系列、照片、首页文案、摄影师资料和 v1.1 内容模块，并同步配置到腾讯云 COS。
...
- 编辑首页轮播文案
- 编辑预约页心仪风格选项
- 编辑内容模块：套餐、档期、客户评价、服务流程、FAQ、咨询按钮、咨询模板、摄影师列表、门店列表
- 编辑摄影师资料和门店信息
- 同步 `portfolio-config.json` 到 COS

当前版本不保存客户咨询信息。小程序端只根据 CMS 模板生成咨询文本，写入用户本机临时缓存，并引导用户复制后发送给摄影师确认档期和方案。
