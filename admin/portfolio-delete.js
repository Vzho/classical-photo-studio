...
function removeHomeFeaturedPhotoReference(series, photoNames) {
  if (!Array.isArray(series.homeFeaturedPhotos)) return

  const removedNames = new Set(photoNames)
  series.homeFeaturedPhotos = normalizeStringList(series.homeFeaturedPhotos)
    .filter(photoName => !removedNames.has(photoName))
...
function removePhotoFromConfig(config, { themeId, seriesId, photoName }) {
  ...
  removeHomeFeaturedPhotoReference(series, [normalizedPhotoName])
...
function reconcileMissingPhotoReferences(config, existingPhotoNames) {
  ...
  for (const theme of nextConfig.themes || []) {
    for (const series of theme.series || []) {
      ...
      removeHomeFeaturedPhotoReference(series, missing)
