...
- 上传摄影师头像并自动写入配置
- 编辑首页轮播文案
- 通过“主题设置”选择预设主题、品牌名、主色、背景色、文字色、卡片风格、按钮风格、图片圆角和布局疏密
- 编辑预约页心仪风格选项
- 结构化编辑作品系列介绍、适合人群、拍摄场景、风格标签和关联内容
- 编辑摄影师资料、联系方式和门店地址
- 结构化编辑 v1.1 高频内容模块：套餐、档期、客户评价、咨询按钮、服务流程、FAQ、多摄影师、多门店
- 编辑 v1.1 其他内容模块：咨询文本模板
- 同步 `portfolio-config.json` 到 COS
- 不保存客户咨询数据；小程序只在用户本机生成可复制的咨询内容

## 内容模块关联规则

- 套餐、档期、客户评价、咨询按钮、服务流程、FAQ、多摄影师和多门店优先使用“内容模块”里的结构化表单维护，保存时会自动同步到 JSON 并上传 COS
- JSON 文本框作为高级预览和批量排查入口；普通客户不需要手写 JSON
- 客户评价图片和团队摄影师头像支持在结构化表单内直接上传，后台会写回 COS key
...
- 套餐 `relatedPhotographerIds`：把摄影师显示到对应套餐详情页，填写摄影师 `id`
...
保存后会写入 `theme` 字段并同步 COS。小程序会应用主色、背景色、文字色、卡片阴影/边框、按钮圆角、图片圆角和页面疏密；本地配置只作为源码模板和断网兜底。
