...
export function getShareContent(config: any, images?: PortfolioItem[]): ShareContent {
  ...
  const featuredCovers = covers
    .filter(item => item.featuredOnHome)
    .sort((left, right) => {
      const leftSort = Number.isFinite(left.homeSort) ? Number(left.homeSort) : Number.MAX_SAFE_INTEGER
      const rightSort = Number.isFinite(right.homeSort) ? Number(right.homeSort) : Number.MAX_SAFE_INTEGER
      return leftSort - rightSort
    })
  const fallback = featuredCovers[0] || covers[0]
    ...
    theme.series.forEach((series: any) => {
      ...
      photos.forEach((photoFileName: string, index: number) => {
        ...
        items.push({
          ...
          featuredOnHome: Boolean(series.featuredOnHome),
