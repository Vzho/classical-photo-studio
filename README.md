...
推荐在管理后台“云端设置”中填写 AppID。后台会把它写入本机专用的 `miniprogram/project.private.config.json`，不会改动 Git 中的通用模板。需要手动配置时，也可以新建该文件：
...
仓库里的 `miniprogram/project.config.json` 应始终保留 `touristappid`；真实 AppID 不提交到 Git。
...
顶部“页面装修”提供受控装修能力：内置东方高定、时尚画册、清透客片、电影叙事、影像艺廊 5 套商业皮肤，并提供杂志封面、高定目录、影像画廊、门店转化 4 种首页结构。皮肤控制品牌气质，结构真正改变照片、文字和服务内容的位置；作品区支持错落图文、瀑布流、整齐卡片、横向滑动、大小图混排，可设置 1–4 列、封面比例、照片留白，并独立控制标题、分类和描述。四栏自动使用纯图片，避免文字拥挤。底部可在作品集、简介、套餐、咨询、门店中启用并排序 2–5 个入口；门店入口直接读取后台已有的地址、营业时间、电话和地图坐标。首页可显示档期和评价快捷入口，并跳到对应内容。保存前使用当前 COS 客片即时预览，点击预览中的模块可直接定位到左侧设置，也可完整恢复打开页面前的设置。选择结构只会重排首页模块并调整展示参数，不会删除照片、文字、分类或模块内容。旧版 6 套皮肤继续兼容已上线配置，但不再作为新装修选项；系统不开放任意 CSS 或 SVG。作品首页、店铺简介、预约咨询、门店信息、套餐列表、套餐详情、作品详情和咨询结果共 8 个页面使用同一套配置；移出的模块保留原内容，之后可重新添加。咨询表单字段也可调整名称、提示、顺序、必填状态和是否显示，不需要修改源码或 JSON。
...
正常使用后台管理即可，不需要手动维护作品配置文件。配置 COS 后，后台直接读取并更新 COS 的 `config/portfolio-config.json`，不会把客户作品和资料回写进仓库里的本地模板。小程序线上始终优先读取 COS 配置，这样客户后续更新作品、头像、Banner 或资料时，不需要重新上传小程序代码。云端请求临时失败时只复用当前 COS 最近一次成功配置；已配置 COS 后不会回退到源码中的其他客户数据。本地 `miniprogram/data/portfolio-config.json` 仅用于未配置 COS 时的开发模板。
...
- 已在后台“云端设置”或 `project.private.config.json` 中配置客户 AppID
...
配置 COS 后，后台保存只更新当前客户的 COS 配置；未配置 COS 时才写入本地开发模板。本阶段暂不包含预发布环境、版本回滚和一键安装，这三项作为后续独立能力开发。
